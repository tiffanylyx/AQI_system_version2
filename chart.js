...
var clip = line_chart.append("defs").append("svg:clipPath")
    .attr("id", "clip")
    .append("svg:rect")
    .attr("width", width2 )
    .attr("height", height2 )
...
.attr("clip-path", "url(#clip)")
  ...
  .on("click", mouseclick)
  .attr("transform", function(d){
    return `translate(${x_line(d[0])+0}, ${fall_area_height+10+y_line(0)})`})
  .selectAll("path")
  .data(function(d) {
    return d[1];})
  .join("path")
    ...
    .attr("transform", function(d){
      for(i in AQI_values){
        if(AQI_values[i].Date == d[0]){
          return `translate(${x_line(d[0])+0}, ${fall_area_height+10+y_line(AQI_values[i].Value)})`}
        }
      })
...
function dragStarted() {
  ...
  var clonedElement = d3.select(this.cloneNode(true)).attr("class","clone");
   line_chart.node().appendChild(clonedElement.node());
...
const lines = line_chart.append("path").attr("clip-path", "url(#clip)")
      .datum(AQI_values)
  .join("path")
  .attr("class","AQI_line")
  .attr("fill", "none")
...
line_chart.append('g').attr("clip-path", "url(#clip)")
.selectAll("line")
  .data(first_date)
  .join("line")
